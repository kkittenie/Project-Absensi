                ...
                @else
                    <div class="card shadow p-4 text-start">
                        <form action="{{ route('guru.izin.store') }}" method="POST"
                              enctype="multipart/form-data" id="izinForm">
                            @csrf
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="alasan" class="form-control" rows="3"
                                          placeholder="Isi alasan izin" required>{{ old('alasan') }}</textarea>
                            </div>
...
<script>
    ...
    if (btnKirim) {
        btnKirim.addEventListener('click', function () {
            ...
            if (tanggalIzin === today && mode !== 'tutup') {
                const jamInfo = mode === 'masuk'
                    ? `Jam absen masuk: ${JAM_MASUK_MULAI}–${JAM_MASUK_SELESAI}`
                    : `Jam absen pulang: ${JAM_PULANG_MULAI}–${JAM_PULANG_SELESAI}`;

                Swal.fire({
                    icon: 'warning',
                    title: 'Sedang Jam Absensi',
                    text: `Kamu tidak bisa mengajukan izin saat ini. ${jamInfo}`,
