            ...
            <form method="GET" action="{{ route('admin.kehadiran.index') }}" class="row align-items-end g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="nama" class="form-control"
                           placeholder="Cari nama guru..." value="{{ $nama ?? '' }}">
                </div>
    ...
    <ul class="nav nav-tabs mb-0" id="kehadiranTab">
        <li class="nav-item">
            <button class="nav-link active d-flex align-items-center gap-2"
                    id="tab-masuk" onclick="switchTab('masuk')">
        ...
        <li class="nav-item">
            <button class="nav-link d-flex align-items-center gap-2"
                    id="tab-pulang" onclick="switchTab('pulang')">
                    ...
                    <tbody>
                        @forelse ($absenMasuk as $absen)
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($absensi?->photo)
                                        <img src="{{ asset('storage/' . $absensi->photo) }}"
                                             width="45" height="45"
                                ...
                                <td>{{ \Carbon\Carbon::parse($absen->tanggal)->locale('id')->isoFormat('dddd, D MMM Y') }}</td>
                    ...
                    <tbody>
                        @forelse ($absenPulang as $absen)
                            @php
                                ...
                                $selisihMenit = $pulangCepat ? $jamPulang->diffInMinutes($batasNormal) : 0;
                            @endphp
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>
                                    @if($absensi?->photo_pulang)
                                        <img src="{{ asset('storage/' . $absensi->photo_pulang) }}"
                                             width="45" height="45"
                                             class="rounded-2 object-fit-cover"
                                             style="cursor:pointer"
                                ...
                                <td>
                                    @if($dataIzin)
                                        <span class="badge bg-info text-dark">{{ ucfirst($dataIzin->jenis_izin) }}</span>
                                    @elseif($pulangCepat)
                                        <span class="badge bg-danger">Pulang Cepat ({{ $selisihMenit }} mnt)</span>
                                    @elseif($absen->lembur_menit > 0)
                                        <span class="badge bg-info text-dark">Lembur {{ $absen->lembur_menit }} menit</span>
                                    @else
                                        <span class="badge bg-success">Tepat Waktu</span>
                                    @endif
                                </td>
